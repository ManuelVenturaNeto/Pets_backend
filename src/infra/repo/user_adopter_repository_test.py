...
def test_insert_user_adopter():
    ...
    new_user_adopter = user_adopter_repository.insert_user_adopter(name, cpf, email, phone_number, address_id, pet_id)
    engine = db_connection.get_engine()

    with engine.connect() as connection:
        query_user_adopter = connection.execute(
            text("SELECT * FROM user_adopters WHERE id=:id"), {"id": new_user_adopter.id}
        ).fetchone()

        connection.execute(text("DELETE FROM user_adopters WHERE id=:id"), {"id": new_user_adopter.id})
        connection.commit()
...
def test_select_user_adopter():
    ...
    data = UserAdoptersModel(id=user_adopter_id, name=name, cpf=cpf, email=email, phone_number=phone_number, address_id=address_id, pet_id=pet_id)
    ...
    with engine.connect() as connection:
        connection.execute(
            text(
                "INSERT INTO user_adopters (id, name, cpf, email, phone_number, address_id, pet_id) VALUES (:id, :name, :cpf, :email, :phone_number, :address_id, :pet_id)"
            ),
            {
                "id": user_adopter_id,
                "name": name,
        ...
        query_user_adopter2 = user_adopter_repository.select_user_adopter(id=data.id, cpf=data.phone_number)
        query_user_adopter3 = user_adopter_repository.select_user_adopter(cpf=data.cpf)
        query_user_adopter4 = user_adopter_repository.select_user_adopter(cpf=data.cpf, email=data.email, phone_number=data.phone_number, address_id=data.address_id, pet_id=data.pet_id)
        query_user_adopter5 = user_adopter_repository.select_user_adopter(pet_id=data.pet_id)

        connection.execute(text("DELETE FROM user_adopters WHERE id=:id"), {"id": data.id})
        connection.commit()
