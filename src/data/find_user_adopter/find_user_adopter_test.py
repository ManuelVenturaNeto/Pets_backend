...
def test_by_user_adopter_id_fail():
    ...
    # testing input
    assert user_adopter_repo.select_user_adopter_params == {}  # pylint: disable=C1803
...
def test_by_user_information():
    ...
    response = find_user_adopter.by_user_information(
        name=attributes["name"], cpf=attributes["cpf"], email=attributes["email"], phone_number=attributes["phone_number"]
    )
