...
class FindUserAdopter(FindUserAdopterInterface):
    ...
    def by_user_information(self, name: str=None, cpf: int=None, email: str=None, phone_number: int=None) -> Dict[bool, List[UserAdopters]]:
        """
        ...
        """
        response = None
        validate_entry =(isinstance(name, str) or isinstance(name, type(None)) or\
                         isinstance(cpf, str) or isinstance(cpf, type(None)) or\
                         isinstance(email, str) or isinstance(email, type(None)) or\
                         isinstance(phone_number, str) or isinstance(phone_number, type(None))) and\
                        (isinstance(name, str) or isinstance(cpf, str) or isinstance(email, str) or isinstance(phone_number, str))

        if validate_entry:
            response = self.user_adopter_repository.select_user_adopter(name=name, cpf=cpf, email=email, phone_number=phone_number)
